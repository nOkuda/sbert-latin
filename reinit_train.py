...
from sbert_latin.sbertlatin import CosineSimilarityLoss, ReinitSBertLatin
...
def _main():
    ...
    outdir = Path(__file__).parent.resolve() / f'reinit_output_{args.reinit}'
    labelled_examples = get_labelled_examples()
    train_model(initial_seeds[0],
                labelled_examples,
                outdir,
                args.reinit,
                epochs=5)
...
def parse_args():
    ...
    p.add_argument('reinit', type=int, help='Number of layers to reinitialize')
    return p.parse_args()
...
def train_model(initial_seed,
    ...
    model = ReinitSBertLatin(bertPath='latin-bert/models/latin_bert/',
                             reinit=reinit)
    model.to(device)
